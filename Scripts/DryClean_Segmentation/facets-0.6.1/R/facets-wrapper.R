...
readSnpMatrix = function(filename_counts,
    ...
    #' read count table
    pileup = data.table::fread(cmd = paste('gunzip -c', filename_counts), key = c('Chromosome', 'Position'))

    #' stop if counts data is empty
    if (nrow(pileup) == 0) {
        stop(paste(filename_counts, 'does not exist or cannot be read properly.'), call. = F)
    }
    ...
    rcmat = as.data.frame(pileup[ii, 1:2])
