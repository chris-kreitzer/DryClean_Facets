...
readSnpMatrix = function(filename_counts,
    ...
    if (perl.pileup) {
        rcmat = scan(filename_counts, what = list(Chromosome = "", Position = 0, NOR.DP = 0, NOR.RD = 0, TUM.DP = 0, TUM.RD = 0), skip = skip)
        ...
        pileup = pileup[File1E <= err.thresh & 
                            File2E <= err.thresh &
                            File1D <= del.thresh & 
                            File2D <= del.thresh &
        ...
        pileup[order(Chromosome, Position)][, list(Chromosome, Position, NOR.DP, TUM.DP, NOR.RD, TUM.RD)]
    }
    
    pileup = pileup[, !c('Alt', 'Ref', 'File1R', 'File1A', 'File1E', 'File1D', 'File2R', 'File2A', 'File2E', 'File2D')]
    ...
    if(!is.null(filename_dryclean)){
        ...
        return(list(pileup = as.data.frame(pileup)))
    }
}
